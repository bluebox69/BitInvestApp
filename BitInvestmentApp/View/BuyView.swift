        ...
        ScrollView {
            ...
            if (portfolio != nil) {
                VStack(spacing: 20) {
                    ...
                    VStack {
                        Text(NSLocalizedString("cashAmount", comment: "Cash Amount"))
                            .font(.largeTitle)
                            .fontWeight(.bold)
                            .padding(.top, 30)

                        
                        Text("$\(totalCash, specifier: "%.2f")")
                            .font(.largeTitle)
                            .fontWeight(.bold)
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        HStack{
                            Text(NSLocalizedString("amountInDollar", comment: "Amount in Dollar"))
                                .font(.headline)
                                .foregroundColor(.primary)
                            Spacer()
                            Button(
                                NSLocalizedString("investAll", comment: "invest all"),
                                action: {
                                    dollarAmount = String(totalCash)
                                }
                            )
                            .foregroundColor(.gray)
                        ...
                        TextField(
                            NSLocalizedString("enterDollarAmount", comment: "Enter Amount in Dollar"),
                            text: $dollarAmount
                        )
                        .padding()
                        ...
                        .onSubmit {
                            viewModel.addInvestment(investmentAmountInDollars: Double(dollarAmount) ?? 0.0, purchaseType: PurchaseType.buy)
                        }
                        
                        //Berechnung der CoinAmount
                        if let amount = Double(dollarAmount), amount > 0 {
                            Text(String.localizedStringWithFormat(
                                NSLocalizedString("receive_bitcoin", comment: "Message showing the estimated amount of Bitcoin the user will receive"),
                                viewModel.calculateBitcoinAmount(dollarAmount: amount)
                            ))
                                .font(.subheadline)
                    ...
                    let buttonColor: Color = (Double(dollarAmount) ?? 0) > 0 ? .lightYellow : .lightRed
                    CustomButton(
                        title: NSLocalizedString("confirm", comment: "Confirm"),
                        backgroundColor: buttonColor,
                        action: {
                            if let amount = Double(dollarAmount), amount > 0, amount <= totalCash {
                                viewModel.addInvestment(
                                    investmentAmountInDollars: amount,
